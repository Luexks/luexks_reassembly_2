...
use crate::shape_types::PortDistribution::{BackwardsFromNextVert, Center, TowardsFromCurrentVert};
...
fn create_square_shapes(shapes: &mut Shapes) {
    shapes.add_unmirrored_shape_from_scales(
        (0..SQUARE_SCALE_COUNT)
            .map(|scale_index| {
                let half_square_length = TOTAL_SCALE * 0.5 * (1.0 + scale_index as f32);
...
fn create_right_triangle_shapes(shapes: &mut Shapes) {
    shapes.add_mirrored_shape_from_scales(
        RIGHT_TRIANGLE_HEIGHT_SCALE_FACTORS
            .iter()
            .map(|height_scale_factor| {
                RIGHT_TRIANGLE_WIDTH_SCALE_FACTORS
                    .iter()
                    .map(|width_scale_factor| {
                        scale_from_vertices_and_port_distributions!(
                            vert!(0.0, 0.0),
